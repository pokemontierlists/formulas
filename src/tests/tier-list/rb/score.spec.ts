...
describe('score formulas (RB)', () => {
  describe('calculateFinalScore', () => {
    it.each([
      ...
      const result = calculateFinalScore(new LibBig(score), new LibBig(modifiers));
  ...
  describe('calculateMatchupScore', () => {
    it.each([
      ...
      const result = calculateMatchupScore(new LibBig(base), new LibBig(weight), new LibBig(mod));
      // Using startsWith because of the high precision of big.js vs previous rounded number
      expect(result.toString().startsWith(expected.substring(0, 10))).toBe(true);
    });
  });
  ...
  describe('calculateRawMatchupWeight', () => {
    ...
    it.each([
      ...
      const result = calculateRawMatchupWeight(scores.map(s => new LibBig(s)));
      expect(result.toString().startsWith(expected.substring(0, 10))).toBe(true);
    });
  });
  ...
  describe('normalizeRawWeight', () => {
    ...
    const expectedWeights = [
      ...
      '0.053477721024117', '0.071422432467464', '0', '0.04956087461433',
      '0.081017819230544', '0', '0.085245370684745', '0.099517674324276',
      '0.073459325565938',
    ];

    const bigRawWeights = rawWeights.map(w => new LibBig(w));

    it.each(rawWeights.map((w, i) => [w, expectedWeights[i]]))(
      'Normalizes %s to %s',
      (rawWeight, expected) => {
